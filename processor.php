...
switch ($_REQUEST['action']) {
	...
	case 'get_stakes':
		function out($title, $stakes) {
			...
			foreach ($stakes as $stake) {
				...
				echo "<li class=\"{$stake->getType()}\">" .
					($mine ? "<b>" : "") .
					username($stake->getUID()) . ': ' . $stake->getStakeScore() .
					(($stake->isPlayed()) ? ' (' . $stake->getScore() . ')' : '') .
					($mine ? "</b>" : "")
					. '</li>';
			}
			echo '</ul></div>';
	...
	case 'load_stakes_for_uid':
		...
		foreach ($stakes as $stake) {
			...
			if (!isset($_REQUEST['full'])) {
				echo '<li>'.$match->getName().':<br/><small>Счёт: '.$match->getScore().'; Ставка: '.
					$stake->getStakeScore().' '.(($stake->getScore() > 0) ? '<b>' : '').'('.
					$stake->getScore().')'.(($stake->getScore() > 0) ? '</b>' : '').'</small></li>';
			} else {
	...
	case 'load_rating':
		...
		<li>
			...
			<a href="#" onClick="userClick(this, <?=$cur_uid?>, <?=$_REQUEST['comp_id']?>);"><?=$user['user']->getSNnbsp()?></a>
			<?if ($cur_uid == userid()) {?></strong><?}?>
			(<?=$user['scores']?>)
