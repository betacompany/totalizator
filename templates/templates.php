...
function show_match(Match $match, $uid) {
    ...
    if ($stake) {
        switch ($stake->getScore()) {
            case 4:
                $scoresAttr = "data-four=\"true\"";
                break;
            case 2:
                $scoresAttr = "data-two=\"true\"";
                break;
            case 1:
                $scoresAttr = "data-one=\"true\"";
                break;
        }
    }
    ?>
    <li class="well" <? if ($withMy) { ?>data-my="true"<? } ?>
        <? if ($withoutMy) { ?>data-without-my="true"<? } ?> <?= $scoresAttr ?>>
        <h2>
            <?= $match->getName() ?>
...
function inner_menu(Match $match, $uid) {
    ...
    <? if ($match->isAvailableFor($uid)): ?>
        <form action="processor.php" class="form-inline">
            <div class="controls">
                ...
                <input name="score1" type="text" class="input-mini" maxlength="2" size="2"
                       placeholder="<?= $match->getCompetitor1()->getName() ?>"/>
                <span>:</span>
                <input name="score2" type="text" class="input-mini" maxlength="2" size="2"
                       placeholder="<?= $match->getCompetitor2()->getName() ?>"/>
    ...
    <? else: ?>
        <div class="row-fluid">
            ...
            <div class="span8">
                <div onclick="showStakes(<?= $match->getId() ?>, this)"
                     class="pull-right btn dropdown-toggle stakes-btn">
                    Все ставки
